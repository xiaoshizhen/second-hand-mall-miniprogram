...
public class OrderInfoController {
    ...
    /**
     * 卖家端：根据卖家id和订单状态分页查询订单列表（管理自己发布商品的订单）
     */
    @GetMapping("/page/seller")
    public Result<PageInfo<OrderInfo>> findSellerPage(@RequestParam(required = false) Long sellerId,
                                                      @RequestParam(required = false) String state,
                                                      @RequestParam(required = false, defaultValue = "1") Integer pageNum,
                                                      @RequestParam(required = false, defaultValue = "10") Integer pageSize) {
        return Result.success(orderInfoService.findSellerPages(sellerId, state, pageNum, pageSize));
    }
