...
Page({
  data:{
    ...
    goodsInfoRecommendList:[],    //推荐商品
    goodsInfoHotList:[],          //热卖商品
    ...
    request({url: '/goodsInfo/page/all?pageNum=1&pageSize=8'}).then(res => {
      if(res.code === '0'){
        ...
        if(!goodsInfoCarouselList || goodsInfoCarouselList.length === 0){
          ...
          if(goodsInfoCarouselList.length>8){
            goodsInfoCarouselList=goodsInfoCarouselList.slice(0,8);
          }
          // 处理图片URL并过滤掉默认图片的商品
          goodsInfoCarouselList = goodsInfoCarouselList.map(item=>{
            ...
            // 过滤掉使用默认图片的商品
            return item.url !== this.data.defaultImageUrl;
          });
        }
        this.setData({
